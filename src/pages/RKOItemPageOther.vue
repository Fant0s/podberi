<template>
  <Head>
    <title>ПодбериРУ: РКО "{{ $route.params.other }}"</title>
  ...
  <section v-else class="card-page rko-page content">
    ...
    >
      ...
      <transition name="all-tariffs">
        <div v-show="showAllRates" class="all-tariffs">
          <div v-for="rate in othersRates" :key="rate" class="rate">
            ...
            <div class="properties">
              ...
              <div class="line">
                ...
                <span class="value">
                  {{ rate.free_transfers_ur }} бесплатно
                </span>
              </div>
            </div>
            <div class="card-btns">
...
export default {
  ...
  watch: {
    "$route.params.other"(newOther, oldOther) {
      ...
      await this.$api
        ...
        .then((response) => {
          ...
          this.othersRates = this.card.rates.filter((rate) => {
            if (rate.name !== this.$route.params.other) {
              return true;
            }
          });
          this.getMain();
